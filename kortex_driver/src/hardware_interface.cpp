...
std::vector<hardware_interface::StateInterface> KortexMultiInterfaceHardware::export_state_interfaces()
{
    ...
    state_interfaces.emplace_back(hardware_interface::StateInterface("reset_fault", "internal_fault", &in_fault_));
...
std::vector<hardware_interface::CommandInterface> KortexMultiInterfaceHardware::export_command_interfaces()
{
    ...
    command_interfaces.emplace_back(hardware_interface::CommandInterface("reset_fault", "command", &reset_fault_cmd_));

    command_interfaces.emplace_back(
        hardware_interface::CommandInterface("reset_fault", "async_success", &reset_fault_async_success_));
    ...
    const std::vector<std::string> &start_interfaces, const std::vector<std::string> &stop_interfaces)
{
    ...
    for (const auto &key : stop_interfaces) {
        for (auto &joint : info_.joints) {
            ...
            if (key == joint.name + "/" + hardware_interface::HW_IF_VELOCITY && joint.name == gripper_joint_name_) {
                continue;
            }
    ...
    if (!stop_modes_.empty() &&
        std::find(stop_modes_.begin(), stop_modes_.end(), StopStartInterface::STOP_EFF) != stop_modes_.end()) {
    ...
    if (!stop_modes_.empty() &&
        std::find(stop_modes_.begin(), stop_modes_.end(), StopStartInterface::STOP_GRIPPER) != stop_modes_.end()) {
        stop_gripper_controller_ = true;
    }
    if (!stop_modes_.empty() &&
        std::find(stop_modes_.begin(), stop_modes_.end(), StopStartInterface::STOP_FAULT_CTRL) != stop_modes_.end()) {
    ...
    if (!start_modes_.empty() &&
        (std::find(start_modes_.begin(), start_modes_.end(), StopStartInterface::START_EFF) != start_modes_.end())) {
    ...
    if (twist_controller_running_ && (start_joint_pos_controller_ || start_joint_eff_controller_) &&
        !stop_twist_controller_) {
    ...
    const vector<std::string> & /*start_interfaces*/, const vector<std::string> & /*stop_interfaces*/)
{
    ...
    if (start_gripper_controller_) {
        ...
        if (!joint_pos_controller_running_ && !joint_eff_controller_running_ && !twist_controller_running_) {
            ...
            base_.SetServoingMode(servoing_mode_hw_);
            arm_mode_ = k_api::Base::ServoingMode::SINGLE_LEVEL_SERVOING;
            RCLCPP_INFO(LOGGER, "Switched to SINGLE_LEVEL_SERVOING mode.");
        }
    }
    ...
    if ((arm_mode_ != k_api::Base::ServoingMode::SINGLE_LEVEL_SERVOING) && !start_joint_pos_controller_ &&
        !start_joint_eff_controller_ && !start_twist_controller_ && !start_gripper_controller_ &&
        !start_fault_controller_) {
...
return_type KortexMultiInterfaceHardware::write(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
    ...
    if (!std::isnan(reset_fault_cmd_) && fault_controller_running_) {
        RCLCPP_WARN_STREAM(
            LOGGER, "Entering to the fault handling steps of the KortexMultiInterfaceHardware::write()...");
        try {
        ...
        RCLCPP_WARN_STREAM(
            LOGGER, "Exiting from the fault handling steps of the KortexMultiInterfaceHardware::write()...");
    }
    ...
    if (in_fault_ == 0.0) {
        if (arm_mode_ == k_api::Base::ServoingMode::SINGLE_LEVEL_SERVOING) {
            ...
            if (twist_controller_running_) {
                ...
                RCLCPP_DEBUG_ONCE(
                    LOGGER, "No controller active in SINGLE_LEVEL_SERVOING mode!");  //FIXME: change to RCLCPP_DEBUG
            }

            // gripper control
            sendGripperCommand(arm_mode_, gripper_command_position_, gripper_speed_command_, gripper_force_command_);
            ...
            if (joint_pos_controller_running_ || joint_eff_controller_running_) {
                ...
                feedback_ = base_cyclic_.RefreshFeedback();
                RCLCPP_DEBUG_ONCE(
                    LOGGER, "No controller active in LOW_LEVEL_SERVOING mode !");  //FIXME: change to RCLCPP_DEBUG
            }
        } else {
            // Keep alive mode - no controller active
            feedback_ = base_cyclic_.RefreshFeedback();
    ...
    if (joint_pos_controller_running_) {
        ...
        for (size_t i = 0; i < actuator_count_; i++) {
            ...
            base_command_.mutable_actuators(static_cast<int>(i))
                ->set_current_motor(arm_commands_efforts_[i] / motor_constants_[i]);
            base_command_.mutable_actuators(static_cast<int>(i))->set_command_id(base_command_.frame_id());
        }
    }
}
    ...
    if (gripper_controller_running_ && !std::isnan(position) && use_internal_bus_gripper_comm_) {
        try {
            if (arm_mode == k_api::Base::ServoingMode::SINGLE_LEVEL_SERVOING) {
                ...
                finger->set_value(static_cast<float>(position / 0.81));  // This values needs to be between 0 and 1
